...
Two middleware functions are provided by default in the `crawler-server.middleware` namespace.
...
See the `:middleware` key of the `app` definition located in the `crawler-server.handler` namespace to manage the enabled middleware.
